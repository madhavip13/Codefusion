...
public class HypothyroidismTest {

	public static WebDriver driver;

	@Test
	public void ThyroidismRecipies() throws InterruptedException, IOException {
		...
		Thread.sleep(2000);
		WebElement hypothyroidismrecipes = driver.findElement(By.xpath("//a[@id='ctl00_cntleftpanel_ttlhealthtree_tvTtlHealtht211']"));
		...
		int Excelcolumn = ExcelReader.getLastColumn("Sheet1");
		System.out.println("Starting column=" + Excelcolumn);
		for (int j = 1; j <= pagesize; j++) {

			// driver.findElement(By.xpath("//div[@style='text-align:right;padding-bottom:15px;']/a["
			// + j + "]")).click();
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@style='text-align:right;padding-bottom:15px;']/a[" + j + "]")));
...
//				Thread.sleep(10000);
				...
				// String morbityDeatils=driver.findElement(By.xpath("//a[text()='High Blood
				// Pressure']")).getText();
				String morbityDetails = "Hypothyroidism";
				...
				String IngreList = driver.findElement(By.xpath("//div[@id='rcpinglist']")).getText();
				System.out.println("cookingTime=" + cookingTime);
				...
				String[][] ExcludeCode = ExcelReader.getData("Sheet1");
				...
				String[][] AddCode = ExcelReader.getData("Sheet2");
				...
				if ((!isElimIngredExists) && (isAddIngredientExists)) {
					...
					excelWriter.WriteData("Sheet1", 0, Excelcolumn++, ID, RecipeName, recipeCatogory, foodCatogory,
							IngreList, PreTime, cookingTime, PrepMethod, NutriValue, morbityDetails, URL);
				}
